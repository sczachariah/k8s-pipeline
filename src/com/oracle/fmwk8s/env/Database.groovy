    ...
    static deployDatabase(script, databaseVersion, dbNamespace) {
        try {
            if (Common.productId != "weblogic") {
                ...
                script.sh "kubectl exec -it ${this.dbPodName} -n ${dbNamespace} -- bash -c \"source /home/oracle/.bashrc; sqlplus sys/Oradoc_db1@ORCLCDB as sysdba <<EOF @\\\$ORACLE_HOME/rdbms/admin/xaview.sql / exit EOF\""
