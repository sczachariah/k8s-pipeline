    ...
    static createEnvConfigMap(script) {
        try {
            ...
            script.sh "cd kubernetes/framework/${Common.productId} && \
                        ...
                        sed -i \"s|%CONNECTION_STRING%|${Database.dbName}.${Domain.domainNamespace}:1521/${Database.dbName}pdb.us.oracle.com|g\" fmwk8s-${Common.productId}-env-configmap.yaml && \
                        sed -i \"s|%DB_HOST%|${Database.dbName}.${Domain.domainNamespace}|g\" fmwk8s-${Common.productId}-env-configmap.yaml && \
                        sed -i \"s|%DB_PORT%|1521|g\" fmwk8s-${Common.productId}-env-configmap.yaml && \
